 ...
 * This displays a user interface of the milk project that could analyze
 * milk data based on user input file. User can display relevant statistics,
 * add/edit/remove existing data, and save the report to an output file.
 * NOTE: all data displayed in this GUI as of 4/21/20 are DUMMY (fake) data.
 * We are currently working on backend data structure so it can analyze and display
 * real statistics soon.
 *
 * Authors: 
...
/**
 * Main - Main GUI that creates and displays a user interactive screen so users
 * can use Milk Analyzer program to analyze the data.
...
public class Main extends Application {
	...
	public void start(Stage primaryStage) throws Exception {
		...
		//String fileName = fileInput.getText(); -> will be used to load the file (methods implemented later)
	...
	public void chooseOptions(Stage primaryStage) {
		...
		BorderPane root = new BorderPane();
		Label message = new Label("Choose an option below to start analyzing your"
				+ " milk data:");
		...
		v1.getChildren().addAll(b1, new Label(""), new Label(""), new Label(""),
				new Label(""), new Label(""), new Label(""), b2);
		...
		v2.getChildren().addAll(b3, new Label(""), new Label(""), new Label(""),
				new Label(""), new Label(""), new Label(""), b4);
	...
	public void oneOrMoreFarmsStats(Stage primaryStage) {
		BorderPane root = new BorderPane();
		Label message = new Label ("Would you like to analyze statistics for a specific farm, "
				+ "or all farms?");
		message.setFont(new Font("Arial", 30));
		VBox mess = new VBox();
	...
	public void oneOrMoreFarmsPercent(Stage primaryStage) {
		BorderPane root = new BorderPane();
		Label message = new Label ("Would you like to see percent share for a specific farm, "
				+ "or all farms?");
		message.setFont(new Font("Arial", 30));
		VBox mess = new VBox();
	...
	public void addOrEditData(String action, Stage primaryStage) {
		BorderPane root = new BorderPane();
		Label prompt = new Label ("Please enter the milk information you would like to "
				+ action + " below: ");
		...
		ObservableList<String> months = FXCollections.observableArrayList("January", "Feburary", "March", "April",
				"May", "June", "July", "August", "September", "October", "November", "December");
		ComboBox<String> cb2 = new ComboBox<String>(months);
		
		ObservableList<String> years = FXCollections.observableArrayList("2011", "2012", "2013", "2014", "2015", "2016",
				"2017", "2018", "2019");
	...
	public void removeData (Stage primaryStage) {
		BorderPane root = new BorderPane();
		Label message = new Label("Please enter the milk information you would like to "
				+ "remove below: ");
		message.setFont(new Font("Arial", 30));
		root.setTop(message);
	...
	public void showStatsOneFarm(Stage primaryStage) {
	    ...
	    ObservableList<String> month = FXCollections.observableArrayList("January", "Feburary", "March", "April",
				"May", "June", "July", "August", "September", "October", "November", "December");
	...
	public void chooseDatesAllFarms(Stage primaryStage) {
		...
		Label message = new Label("Please select the month and year you would like the program to analyze"
				+ " the milk data of: ");
	...
	public void displayStatisticsAllFarms(Stage previousStage) {
		BorderPane root = new BorderPane();
		Label dataMess = new Label("Here's the statistic for the given month and year"
				+ " for all farms: ");
	...
	public void showPercentShareAllFarms(Stage primaryStage) {
		...
		ObservableList<PieChart.Data> pieChartData =
                ...
                new PieChart.Data("Farm 1", 13),
                new PieChart.Data("Farm 2", 25),
                new PieChart.Data("Farm 3", 10),
                new PieChart.Data("Farm 4", 22),
	...
	/**
	 * Generate reports. Shows a window that let user choose which report 
	 * (farm report, annual report, monthly report, date_range report) to generate.
	 * @author Chelsea Chen
	 * @param primaryStage - prev stage
	 */
	...
	/**
	 * Generates annual report that shows total milk weight and percent share 
	 * for all farms in a specific year in a table.
	 * @author Chelsea Chen
	 * @param primaryStage - prev stage
	 */
	...
	/**
	 * Generates monthly report that shows total milk weight and percent share 
	 * of all farms in a specific month and year in a table.
	 * @author Chelsea Chen
	 * @param primaryStage - prev stage
	 */
	...
	/**
	 * Generates date_range report that shows total milk weight and percent share 
	 * from a specific start date to a specific end date for all farms in a table.
	 * @author Chelsea Chen
	 * @param primaryStage - prev stage
	 */
	...
	/**
	 * Let user enter a file name as the output file.
	 * This does not generate the output file itself -> only shows how GUI is displayed
	 * @author Chelsea Chen
	 * @param primaryStage - prev stage
	 */
	...
	/**
	 * Prints out the success message for saving output file 
	 * and goodbye message for the user.
	 * @author Ruiwen Wang
	 * @param primaryStage - prev stage
	 */
	public void successReport(Stage primaryStage) {
		BorderPane root = new BorderPane();
		Label successMessage = new Label ("Your report is successfully generated"
				+ " to the output file you requested." + "\n" + "You can find the"
				+ " file in the same program folder.");
		Label goodBye = new Label ("Thank you so much for using the Milk Analyzer!"
				+ "\n" + "Have a great day!");
		Image smile = new Image("smile.jpg", 50, 50, false, false);
		
		VBox v1 = new VBox();
		v1.getChildren().addAll(successMessage, new Label(""), 
				goodBye, new ImageView(smile));
